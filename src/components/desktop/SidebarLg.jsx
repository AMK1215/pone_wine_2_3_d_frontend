...
const SidebarLg = ({ show, onClose }) => {
  ...
  const navLinks = [
    ...
    {
      img: titok,
      name: content?.nav?.videos?.toUpperCase(),
      link: '/ads-video?tab=AdsVideo',
    },
    {
      img: profile,
      name: content?.profile?.my_profile?.toUpperCase(),
      link: '/information?tab=profile',
    },
    {
      img: money,
      name: content?.wallet?.wallet?.toUpperCase(),
      link: '/information?tab=transfer',
    },
    {
      img: log,
      name: content?.nav?.logs?.toUpperCase(),
      link: '/information?tab=logs&type=deposit',
    },
    {
      img: promotion,
      name: content?.nav?.promotion?.toUpperCase(),
      link: '/promotion',
    },
    {
      img: contact,
      name: content?.nav?.contact?.toUpperCase(),
      link: '/contact',
    },
    { img: ranking, name: '2D', link: '/2d' },
    { img: ranking, name: '3D', link: '/3d' },
    { img: log, name: '3D History', link: '/3d/history' },
  ...
  const items = [
    {
      ...
      link: '/?type=slot',
      value: '2',
      name: content?.game_type?.slot,
    },
    {
      img: fish,
      link: '/?type=arcade',
      value: '4',
      name: content?.game_type?.arcade,
    },
    {
      img: football,
      link: '/?type=table',
      value: '5',
      name: content?.game_type?.table,
    },
    {
      img: card,
      link: '/?type=casino',
      value: '6',
      name: content?.game_type?.casino,
    },
    {
      img: fish,
      link: '/?type=lottery',
      value: '8',
      name: content?.game_type?.lottery,
    },
    {
      img: football,
      link: '/?type=bingo',
      value: '9',
      name: content?.game_type?.bingo,
    },
    {
      img: fish,
      link: '/?type=fishing',
      value: '0',
      name: content?.game_type?.fishing,
    },
  ...
  return (
    <>
      ...
      {/* Sidebar Drawer */}
      <div
        className={`fixed top-0 right-0 h-full w-80 max-w-full bg-[#181c2f] z-50 transition-transform duration-300 ${
          show ? 'translate-x-0' : 'translate-x-full'
        } flex flex-col`}
      >
        <div className='flex items-center justify-end pe-2 border-b border-gray-700'>
          <button
        ...
        <div className='flex-1 overflow-y-auto flex flex-col items-center gap-1 ps-2 pe-2'>
          <Link
            to={'/information?tab=profile'}
            onClick={onClose}
          ...
          <div className='flex items-center gap-2 bg-gray-700 rounded-full px-4 py-2 w-full justify-center'>
            ...
            <span className='text-white font-bold'>
              {Number(user?.balance).toLocaleString()} Ks
            </span>
          </div>
          <Link
            to={'/information?tab=transfer'}
            onClick={onClose}
            className='flex items-center justify-center w-full bg-gradient-to-r from-yellow-400 to-yellow-600 text-gray-900 font-bold py-2 rounded-full mt-2'
          >
          ...
          <div className='flex flex-col gap-1 w-full mt-2'>
            {navLinks.map((item, index) => (
              <Link
                ...
                key={index}
                onClick={onClose}
                className='flex items-center gap-2 py-2 px-4 rounded-xl hover:bg-white/10 transition-colors text-white font-semibold text-base'
              >
          ...
          <div className='flex justify-center my-2'>
            <a
              href={'https://tttgamingmm.site/TTTGaming.apk'}
              target='_blank'
              rel='noreferrer'
              className='inline-block text-center border border-white px-4 py-2 rounded-full text-white no-underline font-semibold hover:bg-white hover:text-gray-900 transition-colors duration-200'
            >
              Download App
            </a>
          </div>
          ...
          <button
            ...
            className='w-full py-2 bg-red-600 hover:bg-red-700 text-white font-bold text-lg rounded-b-xl transition-colors mt-2 mb-14'
          >
            {loading ? (
