...
import PoneWineImg from './assets/img/mobile.png';

export default function PoneWineGame() {
        ...
        <div className='w-full h-full rounded-3xl bg-[#181A29] p-6'>
          ...
          <div className='flex flex-col items-center mb-4'>
            <img
              src={PoneWineImg}
              alt='Pone Wine Game'
              className='w-full max-w-[220px] rounded-2xl object-cover shadow-lg mb-2'
            />
            <h2 className='text-2xl font-extrabold text-yellow-400 mb-2 tracking-wide drop-shadow text-center'>
              Pone Wine Game
            </h2>
          </div>
          {fetchingUser ? (
            ...
            <>
              <div className='mb-4 grid grid-cols-1 gap-2 text-center text-sm text-gray-200'>
                <div>
                  ...
                  <span className='font-mono'>
                    {user.member_account || user.user_name}
                  </span>
                </div>
                <div>
                  <span className='font-semibold text-yellow-300'>
                ...
                <div>
                  ...
                  <span className='font-mono'>100400</span>
                </div>
              </div>
              {error && (
                <div className='mb-2 px-3 py-2 bg-red-100 text-red-700 rounded-lg text-center text-sm font-semibold shadow'>
                  {error}
                </div>
              )}
              {success && (
                <div className='mb-2 px-3 py-2 bg-green-100 text-green-700 rounded-lg text-center text-sm font-semibold shadow'>
                  {success}
                </div>
              )}
              <button
              ...
              >
                {loading ? (
                  <span className='inline-block w-5 h-5 border-2 border-yellow-400 border-t-transparent rounded-full animate-spin align-middle' />
                ) : (
                  'Launch Pone Wine Game'
                )}
              </button>
            </>
          ) : (
            <div className='mb-2 px-3 py-2 bg-red-100 text-red-700 rounded-lg text-center text-sm font-semibold shadow'>
              Failed to load user info. Please log in again.
            </div>
          )}
        </div>
      </div>
